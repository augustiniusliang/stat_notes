...
\begin{linenomath}\begin{equation} {\rho }_{A} = {\operatorname{tr}}_{B}\left( {\rho }_{AB}\right)  = \langle {\left. 0\right| }_{B}{\rho }_{AB}\left| {0{\rangle }_{B}+\langle 1{\left. \right| }_{B}{\rho }_{AB}}\right| 1{\rangle }_{B}  = \operatorname{diag}\left( |\alpha|^2 , |\beta|^2\right).\end{equation}\end{linenomath}
...
统计物理所关心的是大量微观粒子所形成的宏观物体，这些宏观物体内部粒子数众多，这些粒子不仅给物体本身的哈密顿量带来很多自由度，而且和环境也有各式各样的相互作用。因此按照 \ref{partial} 节所述，我们应该将宏观物体和环境的整体看作一个整体，由一个 Hamiltonian $H=H_{A} + H_{\text{环境}} + H_{int}$ 支配。所以我们在取子系统时，应将宏观物体取为关心的子系统，而把环境自由度通过部分迹操作去掉。这样得到了约化的密度矩阵 $\rho.$ 按照我们的讨论，现在物体的量子态是退相干的混合态。因此对于任意一个本征态（在上述近似的意义下），我们有概率\begin{linenomath}\begin{equation}w_n=w(E_n)=\rho_{nn}.\end{equation}\end{linenomath}
...
\begin{linenomath}\begin{equation}\langle H_{int} \rangle \ll H_A, \langle \rho_{ent} \rangle \ll \rho_A.\end{equation}\end{linenomath}
...
\begin{linenomath}\begin{equation}W_{i \to f} = \frac{2\pi}{\hbar} |\langle f | \hat{H}_{int} | i \rangle|^2 \delta(E_f - E_i),\end{equation}\end{linenomath}
...
\subsection{熵与温度}
\label{entropy}
子系统的\textbf{熵}定义为统计权重的对数： \begin{linenomath}\begin{equation}S(E):=k\ln\left( \Delta\Gamma\left( E \right) \right) = -k\ln\left( w(E) \right) = -\sum_n kw(E_n)\ln(w(E_n)).\end{equation}\end{linenomath}
最后一个式子（称为熵的\textbf{ Gibbs 定义式}）是合理的，因为这里定义的熵是一个宏观量，也就是一个时间平均，关于这个时间的的论述可以见\ref{time}节。$k$ 是用来规定量纲的常数。用更一般的密度矩阵语言，我们可以写出\begin{linenomath}\begin{equation}S=-k\tr(\hat\rho\ln\hat\rho).\end{equation}\end{linenomath} 显然可以看到，S的大小表征了能壳内量子态的多少，也就是多少表征了粒子运动的“混乱程度”。

我们定义\textbf{绝对温度}是\begin{linenomath}\begin{equation}T = \frac{1}{\frac{\partial{S}}{\partial{E}}},\end{equation}\end{linenomath} 因此我们见到温度的量纲是 $[E][k]^{-1}.$ 显然，熵 $S$ 、能量 $E$ 及温度 $T$ 在其中一者固定时，另外两者以这样或那样的函数形式互相依赖（或不变）；若其中二者固定，第三者也不变。
...
\label{dist}
\end{equation}\end{linenomath}
其中前面的常数由约束 \begin{linenomath}\begin{equation}\int dw = 1\end{equation}\end{linenomath} 确定。
...
\end{equation}\end{linenomath}
若计及了简并度，那么就有 \begin{linenomath}\begin{equation}dw = Ag\left( E_i \right) \exp\left( -\frac{E_i}{kT} \right) dE_i.\end{equation}\end{linenomath}
...
其中定义了配分函数 \begin{linenomath}\begin{equation}Z = \tr\left( \exp\left( -\frac{\hat{H}}{kT} \right) \right).\end{equation}\end{linenomath} 今后将会看到，配分函数蕴含了一个系统的全部统计信息。
...
\subsection{平均能量的计算}
我们已经有配分函数 \begin{linenomath}\begin{equation}Z=\sum_{s} g(E_s)\exp\left( -\beta{E_s} \right)\end{equation}\end{linenomath}，我们现在就可以计算一些有关系统的参量，对于正则系综来说，这个量就是\textbf{平均能量。}
藉由系综平均的表达式，我们看到平均能量就是 \begin{linenomath}\begin{equation}\bar{E} = \tr\left( \hat H\hat \rho \right) = \frac{1}{Z}\sum_{s} E_sg(E_s)\exp\left( -\beta{E_s} \right) = -\frac{\partial Z}{Z\partial \beta}.\end{equation}\end{linenomath}
...
\begin{linenomath}\begin{equation}\left( \frac{\partial S}{\partial N} \right)_E \equiv -\frac{\mu}{T}.\end{equation}\end{linenomath}
...
正如在正则系综中，我们对于逸度求导可以得到平均能量一样，我们现在也可以通过对巨配分函数的求导得到巨正则分布的特征参量，就是\textbf{平均占有数。}具体来说，我们设参量 \begin{linenomath}\begin{equation}\alpha = -\frac\mu {k_BT},\end{equation}\end{linenomath} 那么平均占有数就可以写成 \begin{linenomath}\begin{equation} \bar N = \tr\left( \hat\rho \hat N \right) = \frac1\Xi \sum_iN_i \exp\left( \alpha N_i-\beta E_i \right) = -\frac{\partial \Xi}{\Xi\partial \alpha}.\end{equation}\end{linenomath} 注意，平均能量的求法和正则系综一样。
...
而对于 Fermi 子，容许的占有数是 $0$ 和 $1.$ 于是巨配分函数就是 \begin{linenomath}\begin{equation}\Xi_s = \left( 1+\exp\left( \frac{\mu-E_s}{kT} \right) \right), \end{equation}\end{linenomath}而占有数期望值是：
\begin{linenomath}\begin{equation}
