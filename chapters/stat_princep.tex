...
这样，量子态就“退相干”了。这意味着子系统 $A$ 已经失去了量子相干性，它不再能发生干涉，其行为完全等同于一个“以 $|\alpha|^2$ 概率处于 $0$ 态，以 $|\beta|^2$ 概率处于 $1$ 态”的\textbf{经典随机系统}。这个事实有一个统计解释：当我们对于大量这样的系统进行平均时，因为环境（子系统 $B$） 的状态我们未加测量，所以我们观测到的子系统状态 $A$ 就是非相干的。这种解释被称作\textbf{系综解释}，这些大量相同系统所组成的集合被称为\textbf{系综}。因此我们对于任意算符$\hat O,$ 在系综下都可以进行平均：
\[
...
从另一个方面也可以导出 Gibbs 分布，这种方面使人更加明了 Gibbs 分布满足熵最大的原理. 现在，我们使用带约束的变分法，将熵作为 Lagrangian：
\[
...
\]
这之中求和是对整个 Fock 空间。同样，巨配分函数也蕴含整个系统的所有统计信息。注意，这里$\rho$ 是否是运动积分取决于 $H$ 与 $N$ 是否对易。我们看到，对于那些不会凭空产生或湮灭粒子（比如 Hamilton 算符中没有类似 $a^\dagger a^\dagger$ 的项）的 Hamilton 算符，这样的条件一定是成立的，而这是大多数情况。

正如在正则系综中，我们对于逸度求导可以得到平均能量一样，我们现在也可以通过对巨配分函数的求导得到巨正则分布的特征参量，就是\textbf{平均占有数。}具体来说，我们设参量 \[\alpha = \frac\mu T,\] 那么平均占有数就可以写成 \[ \bar N = \tr\left( \hat\rho \hat N \right) = \frac1Z \sum_iN_i \exp\left( \alpha N_i-\beta E_i \right) = \frac{\partial \Xi}{\Xi\partial \alpha}.\] 注意，平均能量的求法和正则系综一样。
